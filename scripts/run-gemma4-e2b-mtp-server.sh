#!/usr/bin/env bash
# Gemma 4 E2B Edge target + gemma4_assistant MTP draft (centroid LM head when use_ordered_embeddings=true).
# Override MAIN_GGUF to your converted E2B target GGUF (not bundled in this repo by default).
#
# MTP_PRESET: throughput | lift | balanced | quality — same semantics as run-gemma4-e4b-mtp-server.sh

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SERVER="${LLAMA_SERVER:-${ROOT}/build/bin/llama-server}"
MAIN="${MAIN_GGUF:-${ROOT}/.scratch/gemma-4-e2b/gemma-4-E2B-it-Q4_K_M.gguf}"
_DRAFT_Q4="${ROOT}/.scratch/gemma-e2b-assistant-mtp-Q4_K_M.gguf"
_DRAFT_F16="${ROOT}/.scratch/gemma-e2b-assistant-mtp.gguf"
...
if [[ ! -f "$MAIN" ]]; then
  ...
  echo "hint: convert an E2B-it target GGUF into ${MAIN} or set MAIN_GGUF" >&2
  exit 1
fi
if [[ "$SPEC" == "mtp" ]]; then
  if [[ ! -f "$DRAFT" ]]; then
    echo "error: draft (assistant) GGUF not found: ${DRAFT}" >&2
    echo "hint: hf download google/gemma-4-E2B-it-assistant --local-dir ${ROOT}/.scratch/gemma-4-E2B-it-assistant" >&2
    echo "hint: PYTHONPATH=${ROOT}/gguf-py python3 ${ROOT}/convert_hf_to_gguf.py ${ROOT}/.scratch/gemma-4-E2B-it-assistant --outfile ${_DRAFT_F16} --outtype f16" >&2
    echo "hint: ${ROOT}/scripts/quantize-gemma4-edge-assistant-mtp.sh e2b" >&2
    exit 1
  fi
